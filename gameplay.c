...
void *roomThreadFunc(void *arg)
{
    ...
    initGame(room);
    int playingClientfd, remain_time = REMAIN_TIME, bingo = 0, turn = 0, max_clients = 30, activity, sd, client_socket[30];
    ...
    while (bingo != 1)
    {
        ...
        FD_SET(threadData->master_socket, &threadData->readfds);
        ...
        for (int i = 0; i < room->playerAmount; i++)
        {
            FD_SET(room->player[i]->clientfd, &threadData->readfds);
            if(room->player[i]->clientfd > threadData->max_sd)
                threadData->max_sd = room->player[i]->clientfd;
        }

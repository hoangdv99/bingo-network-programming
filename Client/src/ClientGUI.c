...
char btn_all_name[][MAX_STRING] = {"btn_bingo_ok", "btn_error_ok", "btn_menu_login", "btn_lobby_log_out"
                                    , "btn_lobby_create", "btn_lobby_quick_join", "btn_lobby_join", "btn_room_back", "btn_playing_bingo"
                                    , "btn_invite_accept", "btn_invite_decline", BTN_ROOM_START, BTN_ROOM_READY, BTN_ROOM_KICK, BTN_ROOM_INVITE
                                    , BTN_PLAYING_QUIT, BTN_PLAYING_BACK, "btn_menu_reg"};
char entry_all_name[][MAX_STRING] = {ENTRY_MENU_LOG_USER, ENTRY_LOBBY_SEARCH, ENTRY_ROOM_SEARCH, ENTRY_MENU_LOG_PAS, ENTRY_MENU_REG_PAS
                                    , ENTRY_MENU_REG_USER, ENTRY_MENU_REG_CON_PAS};
char label_all_name[][MAX_STRING] = {"lbl_menu_log_user", "lbl_menu_log_pas", "lbl_menu_reg_user", "lbl_menu_reg_pas", "lbl_menu_reg_con_pas", "lbl_room_id"
                                    , "lbl_menu_reg", "lbl_playing_info", LBL_ERR, LBL_BINGO, LBL_INVITE};
...
int clientGUI(int clientfd) {
    ...
    widgets->clientfd = clientfd;//chuyen cong ket noi vao ben gtk
    ...
    for (int i = 0; i < number_window; i++) gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(gtk_builder_get_object(builder, window_all_name[i]))), "window");
    ...
    for (int i = 0; i < number_entry; i++) gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(gtk_builder_get_object(builder, entry_all_name[i]))), "entry");
    ...
    for (int i = 0; i < number_lable; i++) gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(gtk_builder_get_object(builder, label_all_name[i]))), "label");
    ...
    for (int i = 0; i < number_button; i++) gtk_style_context_add_class(gtk_widget_get_style_context(GTK_WIDGET(gtk_builder_get_object(builder, btn_all_name[i]))), "button");
...
void on_btn_menu_details_clicked(GtkButton *button, app_widgets *app_wdgts) {
    ...
    if (file_success) gtk_text_buffer_set_text(app_wdgts->w_txt_buf, file_contents, -1);
...
void on_btn_lobby_log_out_clicked(GtkButton *button, app_widgets *app_wdgts) {
    ...
    logOut(app_wdgts->clientfd, req, res);
    app_wdgts->currentWindow = app_wdgts->currentWindow - 1;
    gtk_stack_set_visible_child(app_wdgts->w_stack_container, app_wdgts->w_container_list[app_wdgts->currentWindow]);
    gtk_stack_set_visible_child(app_wdgts->w_stack_menu, app_wdgts->w_container_menu_log);
    return;
...
void on_btn_room_kick_clicked(GtkButton *button, app_widgets *app_wdgts) {
    ...
    if (check == FALSE) showError(MES_NO_KICK, app_wdgts->w_lbl_err, app_wdgts->w_err_window);
    return;
}
...
void on_tog_btn_table_toggled(GtkToggleButton *toggle_button, app_widgets *app_wdgts) {
    if (!gtk_toggle_button_get_active(toggle_button)) gtk_toggle_button_set_active(toggle_button, TRUE);
}
